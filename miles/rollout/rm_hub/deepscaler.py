...
def get_deepscaler_rule_based_reward(response, label):
    ...
    # math-verify fallback: string graders miss many equivalent forms
    # (reordered \frac numerators, split fractions, pi factor order ...)
    from .math_verify_fallback import math_verify_grade

    for ground_truth in processed_ground_truths:
        if math_verify_grade(model_answer, ground_truth):
            return 1
