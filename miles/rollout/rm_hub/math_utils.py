...
def grade_answer_union(solution_str, ground_truth):
    """Math-line default grader: grade_answer_verl, then math-verify fallback.

    The fallback only fires when the string graders fail, so it can only flip
    a would-be-0 reward to 1 (monotone).  See math_verify_fallback.py.
    """
    if grade_answer_verl(solution_str, ground_truth):
        return True
    from .math_verify_fallback import math_verify_grade

    given = extract_answer(solution_str)
    gt = str(ground_truth)
    if "\\boxed" in gt:
        gt = extract_answer(gt) or gt
    return math_verify_grade(given, gt)
