...
def _should_retry_same_prompt(args: Namespace, reason: str | None, group: list[Sample]) -> bool:
    if reason not in {"solve_none", "solve_all"}:
        return False
    max_retries = getattr(args, "dynamic_sampling_resample_same_prompt_max_times", 0)
    if max_retries <= 0:
        return False
    retry_count = int(group[0].metadata.get("dynamic_sampling_retry", 0) or 0)
    return retry_count < max_retries


def _clone_group_for_retry(group: list[Sample]) -> list[Sample]:
    retry_count = int(group[0].metadata.get("dynamic_sampling_retry", 0) or 0) + 1
    retried_group = []
    for sample in group:
        retried = copy.deepcopy(sample)
        retried.tokens = []
        retried.response = ""
        retried.response_length = 0
        retried.reward = None
        retried.loss_mask = None
        retried.weight_versions = []
        retried.rollout_log_probs = None
        retried.rollout_routed_experts = None
        retried.multimodal_train_inputs = None
        retried.remove_sample = False
        retried.status = Sample.Status.PENDING
        retried.non_generation_time = 0.0
        retried.spec_info = Sample.SpecInfo()
        retried.prefix_cache_info = Sample.PrefixCacheInfo()
        retried.metadata = dict(retried.metadata)
        retried.metadata.pop("env_error", None)
        retried.metadata["dynamic_sampling_retry"] = retry_count
        retried_group.append(retried)
    return retried_group
...
async def generate(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    if state.processor and sample.multimodal_inputs and any(v is not None for v in sample.multimodal_inputs.values()):
        ...
        prompt_ids = state.tokenizer.encode(sample.prompt, add_special_tokens=True)
    ...
    routed_experts_weights_b64: str | None = None
    meta_info = output.get("meta_info", {}) if isinstance(output, dict) else {}
    if isinstance(meta_info, dict):
        v = meta_info.get("routed_experts_weights", None)
        if isinstance(v, str):
            routed_experts_weights_b64 = v
        elif isinstance(v, list) and len(v) > 0 and isinstance(v[0], str):
            routed_experts_weights_b64 = v[0]
        else:
            customized_info = meta_info.get("customized_info", None)
            if isinstance(customized_info, dict):
                w = customized_info.get("routed_experts_weights", None)
                if isinstance(w, str):
                    routed_experts_weights_b64 = w
                elif isinstance(w, list) and len(w) > 0 and isinstance(w[0], str):
                    routed_experts_weights_b64 = w[0]
            if routed_experts_weights_b64 is None and isinstance(output, dict):
                customized_info = output.get("customized_info", None)
                if isinstance(customized_info, dict):
                    w = customized_info.get("routed_experts_weights", None)
                    if isinstance(w, str):
                        routed_experts_weights_b64 = w
    ...
    if routed_experts_weights_b64 is not None:
        try:
            sample.rollout_routed_experts_weights = np.frombuffer(
                pybase64.b64decode(routed_experts_weights_b64.encode("ascii")),
                dtype=np.float32,
            ).reshape(
                len(sample.tokens) - 1,
                args.num_layers,
            ...
            sample.rollout_routed_experts_weights = None
...
async def generate_and_rm_group(
    ...
    tasks = []
    retry_offset = int(group[0].metadata.get("dynamic_sampling_retry", 0) or 0) * args.n_samples_per_prompt
    for idx, sample in enumerate(group):
        current_sampling_params = sampling_params.copy()
        if getattr(args, "sglang_enable_deterministic_inference", False):
            seed = state.group_sampling_seeds[idx] + retry_offset
...
async def generate_rollout_async(
    ...
    while len(data) < target_data_size:
        while state.remaining_batch_size < target_data_size:
            if retry_groups:
                num_to_submit = min(len(retry_groups), target_data_size - state.remaining_batch_size)
                samples = retry_groups[:num_to_submit]
                del retry_groups[:num_to_submit]
            else:
        ...
        for task in done:
            ...
            if not dynamic_filter_output.keep:
                ...
                if _should_retry_same_prompt(args, dynamic_filter_output.reason, group):
                    retry_groups.append(_clone_group_for_retry(group))
                continue
