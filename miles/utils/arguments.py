        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "rewards WITHOUT the soft overlong punishment. The actor's advantages/returns "
                    "still use the fully shaped reward, with the same critic providing the baseline V."
                ),
            )
            parser.add_argument(
                "--use-rollout-entropy",
            ...
            parser.add_argument(
                ...
                type=str,
                default=None,
                help="Path to the custom TIS/RS function (e.g., miles.backends.training_utils.loss.icepop_function).",
            )
            parser.add_argument(
                "--custom-pg-loss-reducer-function-path",
                type=str,
                default=None,
                help="Path to a custom reducer function for pg_loss only. When set, pg_loss will use this custom reducer while other metrics (pg_clipfrac, ppo_kl, entropy_loss, etc.) still use the default sum_of_sample_mean. The path is resolved like --custom-tis-function-path (module.path:factory).",
            )
