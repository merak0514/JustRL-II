...
class TrackingBackend(ABC):
    ...
    def log_histogram(self, tag: str, values, step: int | None = None) -> None:
        return
...
class WandbBackend(TrackingBackend):
    ...
    def log_histogram(self, tag: str, values, step: int | None = None) -> None:
        ...
        wandb.log({tag: wandb.Histogram(values.numpy() if hasattr(values, "numpy") else values)}, step=step)
...
class TensorboardBackend(TrackingBackend):
    ...
    def log_histogram(self, tag: str, values, step: int | None = None) -> None:
        if self._adapter is not None:
            self._adapter.log_histogram(tag, values, step)
...
class SwanlabBackend(TrackingBackend):
    def init(self, args, *, primary: bool = True, **kwargs) -> None:
        from . import swanlab_utils

        swanlab_utils.init_swanlab(args, primary=primary, **kwargs)

    def log(self, metrics: dict[str, Any], step: int | None = None, step_key: str | None = None) -> None:
        from . import swanlab_utils

        swanlab_utils.log_metrics(metrics, step=step, step_key=step_key)

    def log_histogram(self, tag: str, values, step: int | None = None) -> None:
        from . import swanlab_utils

        swanlab_utils.log_histogram(tag, values, step=step)

    def finish(self) -> None:
        from . import swanlab_utils

        swanlab_utils.finish()
...
BACKEND_REGISTRY: dict[str, tuple[type[TrackingBackend], str]] = {
    ...
    "swanlab": (SwanlabBackend, "use_swanlab"),
...
class TrackingManager:
    ...
    def log_histogram(self, tag: str, values, step: int | None = None) -> None:
        for backend in self._backends:
            backend.log_histogram(tag, values, step=step)
