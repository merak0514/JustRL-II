...
def init_process_group(
    ...
    # Use proper version parsing (not lexicographic string compare, which wrongly
    # treats "2.10"/"2.11" as < "2.6").
    torch_base_version = parse_version(str(torch.__version__).split("+")[0])
    pg_options_param_name = "backend_options" if torch_base_version >= parse_version("2.6") else "pg_options"
