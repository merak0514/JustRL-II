...
def convert_minimax_m2_to_hf(args, name, param):
    """Convert MiniMax-M2/M2.5 MoE weights from Megatron to HuggingFace format.

    HF naming uses block_sparse_moe with w1/w2/w3 expert weights,
    sigmoid routing gate, and e_score_correction_bias.
    """
    ...
    if match:
        ...
        if match:
            ...
            if rest == "linear_fc1":
                w1_weight, w3_weight = param.chunk(2, dim=0)
                return [
                    (f"model.layers.{layer_idx}.block_sparse_moe.experts.{expert_idx}.w1.weight", w1_weight),
                    (f"model.layers.{layer_idx}.block_sparse_moe.experts.{expert_idx}.w3.weight", w3_weight),
                ]
