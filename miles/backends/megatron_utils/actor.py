...
from miles.utils.memory_utils import clear_memory, print_memory, print_memory_extended
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        # Lazily-built iterator for --save-hf raw exports. Separate from the one
        # inside weight_updater: archives must stay unquantized even when the
        # rollout side runs fp8/int4 (quantization_config=None).
        self._save_hf_weight_iterator = None
    ...
    def sleep(self) -> None:
        ...
        print_memory_extended("sleep: after torch_memory_saver.pause (train model offloaded)", clear_before_print=True)
    ...
    def wake_up(self) -> None:
        assert self.args.offload_train
        print_memory_extended("wake_up: before torch_memory_saver.resume (train model on CPU)", reset_peak=True)
    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch) -> None:
        print_memory_extended(f"train_actor[{rollout_id}]: entry (after wake_up)", reset_peak=True)
        ...
        with inverse_timer("train_wait"), timer("train"):
            if self.args.compute_advantages_and_returns:
                if "ref" in self.weights_backuper.backup_tags:
                    ...
                    print_memory_extended(f"train_actor[{rollout_id}]: before ref_log_probs forward")
                    rollout_data.update(
                    ...
                    print_memory_extended(f"train_actor[{rollout_id}]: after ref_log_probs forward")
                ...
                if not self.args.use_rollout_logprobs or self.args.get_mismatch_metrics or self.args.monitor_logprob_diff:
                    for m in all_replay_managers:
                        if m.enabled:
                            if self._use_rollout_replay(m):
                                ...
                                m.stage = "record"
                    print_memory_extended(f"train_actor[{rollout_id}]: before actor_log_probs forward")
                    rollout_data.update(
                        self.compute_log_prob(
                            data_iterator,
                            num_microbatches,
                    ...
                    print_memory_extended(f"train_actor[{rollout_id}]: after actor_log_probs forward")
                    for m in all_replay_managers:
                        if self._use_rollout_replay(m):
                            m.clear_all_forward()
            ...
            # Free cached CUDA memory from the log_probs / advantage phases
            # before training to reduce fragmentation pressure.
            clear_memory()

            # Train
            self._set_replay_stage("replay_backward")
            print_memory_extended(f"train_actor[{rollout_id}]: before actor_train (fwd+bwd+optim)", reset_peak=True)
            ...
            print_memory_extended(f"train_actor[{rollout_id}]: after actor_train peak")
    ...
    def _should_save_hf(self, rollout_id: int) -> bool:
        if self.args.save_hf is None or self.role != "actor":
            return False
        if self.args.save_hf_interval is None:
            return True
        # save_model (and the export inside it) is collective, so this decision
        # must be a pure function of rollout_id and static args: every rank has
        # to reach the same answer or the collectives deadlock.
        return (rollout_id + 1) % self.args.save_hf_interval == 0 or rollout_id == self.args.num_rollout - 1

    def _save_hf(self, rollout_id: int) -> None:
        if self.args.megatron_to_hf_mode != "raw" or is_lora_enabled(self.args):
            # LoRA needs adapter merging; both cases go through Megatron-Bridge.
            from miles.backends.megatron_utils.model import save_hf_model

            save_hf_model(self.args, rollout_id, self.model)
            return

        from .hf_export import save_hf_model_raw
        from .update_weight.hf_weight_iterator_base import HfWeightIteratorBase

        with timer("save_hf"):
            if self._save_hf_weight_iterator is None:
                self._save_hf_weight_iterator = HfWeightIteratorBase.create(
                    self.args,
                    self.model,
            ...
            save_hf_model_raw(
                self.args,
                rollout_id,
                hf_weight_iterator=self._save_hf_weight_iterator,
                weights_getter=lambda: self.weights_backuper.get("actor"),
            )
    ...
    def update_weights(self) -> None:
        ...
        with torch_memory_saver.disable() if self.args.offload_train else nullcontext():
            print_memory_extended("update_weights: before weight sync to SGLang", reset_peak=True)
            self.weight_updater.update_weights()
            print_memory_extended("update_weights: after weight sync to SGLang")
