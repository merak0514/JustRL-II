...
from miles.utils.rollout_mbu_utils import SGLangMetricsPoller
...
class SGLangEngine(RayActor):
    ...
    def get_metrics_text(self, timeout: float = 5.0) -> str:
        if self.node_rank != 0:
            return ""
        response = requests.get(
            f"http://{self.server_host}:{self.server_port}/metrics",
            timeout=timeout,
        )
        response.raise_for_status()
        return response.text

    def start_rollout_metrics_collection(self, poll_interval_s: float = 1.0) -> dict[str, object]:
        if self.node_rank != 0:
            return {"enabled": False, "error": "rollout metrics collection only runs on node_rank=0"}

        existing = getattr(self, "_rollout_metrics_poller", None)
        ...
        timeout = max(5.0, poll_interval_s * 2.0)
        self._rollout_metrics_poller = SGLangMetricsPoller(
            fetch_metrics_text=lambda: self.get_metrics_text(timeout=timeout),
            poll_interval_s=poll_interval_s,
        )
        return self._rollout_metrics_poller.start()

    def stop_rollout_metrics_collection(self) -> dict[str, object]:
        poller = getattr(self, "_rollout_metrics_poller", None)
        if poller is None:
            return {"enabled": False, "error": "rollout metrics collector is not running"}
        try:
            return poller.stop()
        finally:
            self._rollout_metrics_poller = None
