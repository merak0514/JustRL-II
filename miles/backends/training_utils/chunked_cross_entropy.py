...
def _parallel_size(parallel_state: "ParallelState", name: str) -> int:
    """Support both new GroupInfo fields and legacy flat ParallelState fields."""
    group_info = getattr(parallel_state, name, None)
    if group_info is not None and hasattr(group_info, "size"):
        return group_info.size

    legacy_name = f"{name}_size"
    if hasattr(parallel_state, legacy_name):
        return getattr(parallel_state, legacy_name)

    raise AttributeError(f"ParallelState has neither {name}.size nor {legacy_name}")


def _parallel_group(parallel_state: "ParallelState", name: str):
    """Support both new GroupInfo fields and legacy flat ParallelState fields."""
    group_info = getattr(parallel_state, name, None)
    if group_info is not None and hasattr(group_info, "group"):
        return group_info.group

    legacy_name = f"{name}_group"
    if hasattr(parallel_state, legacy_name):
        return getattr(parallel_state, legacy_name)
...
try:
    from flash_attn.ops.triton.cross_entropy import (
        cross_entropy_loss as _flash_cross_entropy,
    )
...
def _keep_low_precision_logits(*, with_entropy: bool) -> bool:
    ...
    return os.environ.get(
        "MILES_CHUNKED_LOGPROBS_KEEP_BF16", "0"
...
def _project_to_fp32_logits(
    ...
    if tp_size == 1:
        ...
        return _project_chunk_to_logits(
            hidden_states, weight, temperature, keep_low_precision=False,
        )
...
def _chunk_log_probs_fn(
    ...
    logits = _project_chunk_to_logits(
        hs, weight, temperature, keep_low_precision=keep_low_precision,
    )
...
def _log_probs_no_grad_tp1(
    ...
    *,
    ...
    for s in range(0, R, chunk_size):
        e = min(s + chunk_size, R)
        logits = _project_chunk_to_logits(
            hidden_states[s:e], weight, temperature, keep_low_precision=keep_lp,
        )
...
def _chunked_log_probs_tp_parallel(
    ...
    def _project_lp_only(
        _hs: torch.Tensor, _tk: torch.Tensor,
    ) -> torch.Tensor:
        ...
        try:
            ...
            lp, _ = calculate_log_probs_and_entropy(
                logits, _tk, tp_group, with_entropy=False,
                chunk_size=-1, true_on_policy=true_on_policy,
            )
        finally:
            if _was_sp:
    ...
    def _project_and_compute_no_grad(
        _hs: torch.Tensor, _tk: torch.Tensor, _want_ent: bool,
    ) -> tuple[torch.Tensor, torch.Tensor | None]:
        _was_sp = getattr(output_layer, 'sequence_parallel', False)
        if _was_sp:
            output_layer.sequence_parallel = False
        try:
            ...
            lp, ent = calculate_log_probs_and_entropy(
                logits, _tk, tp_group, with_entropy=_want_ent,
                chunk_size=-1, true_on_policy=true_on_policy,
            )
            del logits
        finally:
    ...
    for s in range(0, R, chunk_size):
        ...
        if torch.is_grad_enabled():
            lp_parts.append(
                ckpt(_project_lp_only, hs_c, tk_c, use_reentrant=False)
            )
        else:
...
def _compute_entropy_chunked(
    ...
    for s in range(0, R, chunk_size):
        ...
        logits = _project_to_fp32_logits(
            hidden_states[s:e], output_layer, output_weight, tp_size, temperature,
        )
        if tp_size == 1:
    ...
    if tp_size == 1:
        ...
        if torch.is_grad_enabled():
            log_probs = _chunked_log_probs_tp1_with_grad(
                hidden_states, labels, weight, chunk_size, temperature,
            ...
            if with_entropy:
                with torch.no_grad():
                    entropy = _compute_entropy_chunked(
                        hidden_states, output_layer, output_weight,
                        chunk_size, temperature, tp_size, tp_group,
                    )
        else:
            log_probs, entropy = _log_probs_no_grad_tp1(
                hidden_states, labels, weight, chunk_size, temperature, with_entropy,
                keep_low_precision=keep_low_precision,
            )
    else:
        if torch.is_grad_enabled():
            log_probs, _ = _chunked_log_probs_tp_parallel(
                hidden_states, labels, output_layer, output_weight,
                chunk_size, temperature, tp_group,
                true_on_policy=true_on_policy, with_entropy=False,
            )
            entropy = None
            if with_entropy:
                with torch.no_grad():
            ...
            log_probs, entropy = _chunked_log_probs_tp_parallel(
                hidden_states, labels, output_layer, output_weight,
                chunk_size, temperature, tp_group,
                true_on_policy=true_on_policy, with_entropy=with_entropy,
            )
...
def chunked_log_probs_from_hidden_states(
    ...
    *,
    ...
    tp_size = _parallel_size(parallel_state, "tp")
    cp_size = _parallel_size(parallel_state, "cp")
    tp_group = _parallel_group(parallel_state, "tp")
    ...
    if sp_enabled:
        ...
        hs = gather_from_sequence_parallel_region(
            hs, tensor_parallel_output_grad=True, group=tp_group,
        )
        assert hs.size(0) == seq_len_before * tp_size, (
            f"SP gather: expected {seq_len_before * tp_size} tokens, "
            f"got {hs.size(0)}"
        )
    ...
    try:
        ...
        for i, (tokens, total_length, response_length) in enumerate(
            ...
            if cp_size == 1:
                ...
                cp_chunk_sz, chunks_offset, logits_offset, tokens_offset = (
                    get_logits_and_tokens_offset_with_cp(
                        total_length, response_length, parallel_state,
                        qkv_format, max_seq_len,
                    )
                )
                ...
                hs_0 = hs_0[
                    logits_offset[0][0] - chunks_offset[0][0] :
                    logits_offset[0][1] - chunks_offset[0][0]
                ]
                tokens_0 = tokens[tokens_offset[0][0] : tokens_offset[0][1]]

                hs_1 = hs_1[
                    logits_offset[1][0] - chunks_offset[1][0] :
                    logits_offset[1][1] - chunks_offset[1][0]
                ]
                tokens_1 = tokens[tokens_offset[1][0] : tokens_offset[1][1]]

                assert hs_0.size(0) == tokens_0.size(0), (
                    f"CP chunk 0: hs {hs_0.size(0)} vs tokens {tokens_0.size(0)}"
                )
                assert hs_1.size(0) == tokens_1.size(0), (
                    f"CP chunk 1: hs {hs_1.size(0)} vs tokens {tokens_1.size(0)}"
                )
