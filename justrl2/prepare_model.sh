...
#   MiniCPM5-2.6B/               HF checkpoint  (openbmb/MiniCPM5-2.6B)      -> HF_MODEL_DIR
#   MiniCPM5-2.6B-torch_dist/    Megatron ckpt, TP1                          -> MEGATRON_MODEL_PATH
#   MiniCPM5-2.6B-DSpark-5L/     optional 5-layer draft for DSpark            -> DSPARK_DRAFT_MODEL_PATH
#
...
HF_BASE=${HF_BASE:-openbmb/MiniCPM5-2.6B}
HF_DRAFT=${HF_DRAFT:-openbmb/MiniCPM5-2.6B-DSpark-5L}
WITH_DSPARK=${WITH_DSPARK:-0}
mkdir -p "$MODELS_DIR"

hf download "$HF_BASE" --local-dir "$MODELS_DIR/MiniCPM5-2.6B"
if [ "$WITH_DSPARK" = 1 ]; then
  hf download "$HF_DRAFT" --local-dir "$MODELS_DIR/MiniCPM5-2.6B-DSpark-5L"
fi

if [ -f "$MODELS_DIR/MiniCPM5-2.6B-torch_dist/latest_checkpointed_iteration.txt" ]; then
  ...
  PYTHONPATH=.:Megatron-LM:mbridge python tools/convert_hf_to_torch_dist.py \
    "${MODEL_ARGS[@]}" \
    --hf-checkpoint "$MODELS_DIR/MiniCPM5-2.6B" \
    --save "$MODELS_DIR/MiniCPM5-2.6B-torch_dist"
fi

echo
echo "export MODELS_DIR=$MODELS_DIR    # train.sh defaults derive from this"
[ "$WITH_DSPARK" = 1 ] && echo "export DSPARK_DRAFT_MODEL_PATH=$MODELS_DIR/MiniCPM5-2.6B-DSpark-5L"
true
