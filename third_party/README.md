...
`justrl2/model_args/minicpm5-2b.sh` mirrors this, though miles overwrites the Megatron-side
`max_position_embeddings` with `--seq-length`, so that particular line is inert). No custom
modeling code is needed on
