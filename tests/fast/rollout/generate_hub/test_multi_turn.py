...
class TestRoutedExpertsMultiTurn:
    ...
    def test_tool_response_pads_routed_experts(self, variant, generation_env):
        if variant != "multi_turn_single_sample":
            pytest.skip("This padding is needed for the returned accumulated multi-turn sample")
        ...
        routed_experts_len = len(S.FIRST_PROMPT_TOKEN_IDS) + token_len(S.FIRST_RESPONSE) - 1
        routed_experts = np.arange(routed_experts_len * num_layers * moe_router_topk, dtype=np.int32).reshape(
            routed_experts_len, num_layers, moe_router_topk
        )
        ...
        tool_response_len = token_len(S.FIRST_TOOL_RESPONSE)
        assert sample.rollout_routed_experts is not None
        assert sample.rollout_routed_experts.shape[0] == len(sample.tokens) - 1
        np.testing.assert_array_equal(sample.rollout_routed_experts[:-tool_response_len], routed_experts)
        np.testing.assert_array_equal(sample.rollout_routed_experts[-tool_response_len:], -1)
